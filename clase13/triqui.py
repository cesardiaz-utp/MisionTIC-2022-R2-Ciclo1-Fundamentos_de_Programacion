...
def estadoDelJuego(matriz)->int:
    ...
    if 3 in np.sum(matriz, axis=0) \
            or 3 in np.sum(matriz, axis=1) \
            or np.sum(np.diagonal(matriz)) == 3 \
            or np.sum(np.diagonal(np.fliplr(matriz))) == 3:
        salida = 1
    else:
        #devuelve 2 si gana la Y
        if 30 in np.sum(matriz, axis=0)  \
                or 30 in np.sum(matriz, axis=1) \
                or np.sum(np.diagonal(matriz)) == 30 \
                or np.sum(np.diagonal(np.fliplr(matriz))) == 30:
            salida = 2
        else:
...
def jugarTriqui():
    ...
    while(estado == 4):
        ...
        #convertir la notación de los jugadores a los indices de la matriz
        x=int(tupla[0]) - 1
        y=int(tupla[2]) - 1
        ...
        if x>=0 and x<= 2 and y>=0 and y<=2:
            ...
            if matriz[x,y] == 0:
                if turno:
                    ...
                    matriz[x,y] = 10
                #llamar la función del estado del juego
                ...
                print("Recuerde jugar en casillas vacías!")
        else:
